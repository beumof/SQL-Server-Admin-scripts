...
SELECT bu.database_name AS Database, recovery_model_Desc AS [Recovery_Model],[Full],[Differential],[Transaction_Log]
FROM (
...
) AS SourceTable PIVOT (max(lastBackup) FOR type IN ([Full],[Differential],[Transaction Log])) As bu
